...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function truncateTextByWords(text, maxWords) {
        if (!text || text.trim() === '') return 'Описание отсутствует';
    ...
    function displayProducts() {
        ...
        if (totalProductsInCompany === 0) {
            ...
            currentProducts.slice(0, displayedProducts).forEach((product, index) => {
                ...
                const hasComposition = product.composition && product.composition.trim() !== '';
                const showDescriptionMore = hasDescription && product.description.split(/\s+/).length > MAX_WORDS;
                const showCompositionMore = hasComposition && product.composition.split(/\s+/).length > MAX_WORDS;
                        ...
                        <div class="product-content">
                            ...
                            <div class="company-header">
                                <div class="company-name">
                                    <i class="fas fa-building"></i> ${currentCompany.name}
                                </div>
                                <div class="company-stats">
                                    <span class="company-id">ID: ${currentCompany.eventId}</span>
                                </div>
                            </div>
                            ` : ''}
                            ...
                            <div class="description-section">
                                ...
                                <div class="description-text">
                                    <div class="short-text" id="short-description-${product.id}">
                                        ${shortDescription}
                                        ${showDescriptionMore ? `<span class="show-more-btn" data-product-id="${product.id}" data-type="description">подробнее&gt;&gt;</span>` : ''}
                                    </div>
                                    <div class="full-text" id="full-description-${product.id}" style="display: none;">
                                        ${product.description || 'Описание отсутствует'}
                                        <span class="show-less-btn" data-product-id="${product.id}" data-type="description">&lt;&lt;скрыть</span>
                                    </div>
                                </div>                         
                            </div>
                            ...
                            <div class="composition-section">
                                ...
                                <div class="composition-text">
                                    <div class="short-text" id="short-composition-${product.id}">
                                        ${shortComposition}
                                        ${showCompositionMore ? `<span class="show-more-btn" data-product-id="${product.id}" data-type="composition">подробнее&gt;&gt;</span>` : ''}
                                    </div>
                                    <div class="full-text" id="full-composition-${product.id}" style="display: none;">
                                        ${product.composition || 'Состав отсутствует'}
                                        <span class="show-less-btn" data-product-id="${product.id}" data-type="composition">&lt;&lt;скрыть</span>
                                    </div>
                                </div>
                            </div>
    ...
    function displayPagination(containerElement, currentCompany, displayedProducts, totalCompanies, totalProductsInCompany) {
        ...
        const endProduct = displayedProducts;
        ...
        <div class="top-pagination">
            <div class="pagination-info">
                ...
                <div class="pagination-subinfo">
                    Товар ${totalProductsInCompany > 0 ? `1-${endProduct} из ${totalProductsInCompany}` : 'нет товаров'}
                    <span class="company-counter">Компания ${currentCompanyNumber} из ${totalCompanies}</span>
                </div>
            </div>
        ...
        if (companyNumbersContainer) {
            ...
            // Показываем все номера компаний
            ...
            // Добавляем обработчики для номеров компаний
    ...
    function goToCompany(companyIndex) {
        ...
        // Прокручиваем к верху контейнера с товарами
        productsContainer.scrollIntoView({ behavior: 'smooth' });
    }

    // Функция для добавления обработчиков событий
    function addEventListeners() {
        // Обработчики для кнопок "подробнее" в описании и составе
        document.querySelectorAll('.show-more-btn').forEach(button => {
            button.addEventListener('click', function (e) {
                ...
                const shortElement = document.getElementById(`short-${type}-${productId}`);
                const fullElement = document.getElementById(`full-${type}-${productId}`);
                
                if (!shortElement || !fullElement) return;
        ...
        // Обработчики для кнопок "скрыть" в описании и составе
